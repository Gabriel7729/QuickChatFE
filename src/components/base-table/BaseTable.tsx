...
import ExportTableMenuButton from "./export-table-menu-button/ExportTableMenuButton";
...
function BaseTable<T>({
    ...
    return sortedData.map((item, index) => (
      <Table.Tr key={String((item as any)["id"])}>
        {config.headers.map((header) => {
          ...
          const handleClick = () => config.onClick?.(item);

          return (
            <Table.Td
              key={String(header.key)}
              onClick={handleClick}
              className={config.onClick ? "clickable" : ""}
            >
              {cellContent}
            </Table.Td>
          );
  ...
  return (
    <ScrollArea>
      <Text fw={"500"} c={"#085b68"} size="xl">{config.titleTable}</Text>
